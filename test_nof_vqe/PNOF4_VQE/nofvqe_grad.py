...
class NOFVQE():
    ...
    def _mo_integrals(self, crds_ang):
        ...
        mol = qml.qchem.Molecule(self.symbols, crds_ang, unit="bohr")
    ...
    def ene_vqe(self):
        E_history, params_history, rdm1_history = self._vqe(self.ene_pnof4, self.init_param, self.crd)
        return E_history, params_history, rdm1_history

    def nuclear_gradient_fedorov(self, params, crds, rdm1_opt, h=1e-3):
        """
    ...
    def nuclear_gradient(self, params, crds, h=1e-3, warm_start=True):
        """
...
if __name__ == "__main__":
    ...
    cal = NOFVQE(xyz_file, functional="PNOF4", conv_tol=1e-7, init_param=0.1)

    # Run VQE
    E_history, params_history, rdm1_history = cal.ene_vqe()
    ...
    grad_fedorov = cal.nuclear_gradient_fedorov(params_opt, cal.crd, rdm1_history[-1], h=5e-3)
    print("Nuclear gradient (Fedorov):\n", grad_fedorov)

    # Nuclear gradient at optimized geometry (finite differences)
    grad = cal.nuclear_gradient(params_opt, cal.crd, h=5e-3)
